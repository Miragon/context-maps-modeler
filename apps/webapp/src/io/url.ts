...
export function encodeDocumentToHash(doc: CmDocument): string {
  const json = serializeDocument(doc, false);
  return LZString.compressToEncodedURIComponent(json);
}

export function buildShareUrl(doc: CmDocument, base?: string): string {
  const origin =
    base ??
...
export function decodeDocumentFromHash(hash: string): CmDocument | null {
...
export function readDocumentFromLocation(): CmDocument | null {
 ...
 */
export function writeDocumentToLocation(doc: CmDocument): boolean {
