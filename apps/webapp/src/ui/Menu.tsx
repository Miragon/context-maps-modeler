...
import { SAMPLE_DOCUMENT, emptyDocument } from "@miragon/context-maps-schema-model";
import { exportJson, importJsonFile } from "@/io/json";
import { exportCml, importCmlFile } from "@/io/cml";
...
export function Menu() {
  ...
  const onFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    e.target.value = "";
    if (!file) return;
    const isCml = file.name.toLowerCase().endsWith(".cml");
    const result = isCml ? await importCmlFile(file) : await importJsonFile(file);
    if (result.ok) {
      modeler.importDocument(result.document);
      const notes = (result as { diagnostics?: string[] }).diagnostics;
      toast(
        notes && notes.length
          ? `Imported CML (${notes.length} note(s) skipped)`
          : "Diagram imported",
        "success",
      );
    } else {
  ...
  const onExportCml = () => {
    try {
      exportCml(modeler.exportDocument());
      toast("Exported CML", "success");
    } catch (err) {
      toast(err instanceof Error ? err.message : "Export failed", "error");
    }
  };
  ...
  return (
    <div className="tt-menu" ref={rootRef}>
        ...
        <div className="tt-menu__dropdown" role="menu">
          <label className="tt-menu__title">
            ...
            <CommitInput
              ...
              onCommit={setTitle}
              placeholder="Untitled context map"
            />
          </label>
      ...
      <input
        ref={fileInput}
        type="file"
        accept=".json,application/json,.cml"
        hidden
