 ...
 * mirrored value diverges from the vendored CSS — so re-copying the token file from
 * `Miragon/corporate-identity` with a changed value forces `MIRAGON`/`CD_TOKENS` to be updated too,
 * instead of silently drifting. See CLAUDE.md § "Design system (mandatory)".
 */
